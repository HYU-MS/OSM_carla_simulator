...
# test
specific_spawn_point.location.x = 20
specific_spawn_point.location.y = 13.3
specific_spawn_point.rotation.yaw = 180
...
cars = ['vehicle.nissan.patrol_2021', 'vehicle.tesla.model3', 'vehicle.audi.etron', 'vehicle.mercedes.coupe', 'vehicle.mercedes.coupe_2020',
        'vehicle.bmw.grandtourer', 'vehicle.mercedes.sprinter', 'vehicle.lincoln.mkz_2020','vehicle.carlamotors.carlacola', 'vehicle.nissan.patrol_2021',
        'vehicle.toyota.prius', 'vehicle.nissan.patrol_2021', 'vehicle.bmw.grandtourer', 'vehicle.mercedes.coupe_2020', 
        'vehicle.mercedes.sprinter', 'vehicle.tesla.model3', 'vehicle.mercedes.coupe_2020', 'vehicle.mercedes.coupe',
...
# Spawn in a row for test
test_actor = []

for i in range(0,17,2):
    spawn_point = spawn_points[0]
    spawn_point2 = spawn_points[1]

    spawn_point.location.x = -41.5
    spawn_point.location.y = 114 - 5*i
    spawn_point.rotation.yaw = -90
    spawn_point2.location.x = -44.5
    spawn_point2.location.y = 114 - 5*i
    spawn_point2.rotation.yaw = -90

    car_actor = world.spawn_actor(car_blueprints[i], spawn_point)
    car_actor2 = world.spawn_actor(car_blueprints[i+1], spawn_point2)
    test_actor.append(car_actor)
    test_actor.append(car_actor2)

# spawn_point = spawn_points[0]
# spawn_point.location.x = 0
# spawn_point.location.y = 16.2
# spawn_point.rotation.yaw = 180
# vehicle_next = world.spawn_actor(car_blueprints[-2], spawn_point)
# test_actor.append(vehicle_next)
...
left_display_thread = threading.Thread(target=display_camera_stream, args=(left_camera_data, 'OSM'))
# right_display_thread = threading.Thread(target=display_camera_stream, args=(right_camera_data, 'Right Side View Mirror'))
right_display_thread = threading.Thread(target=display_camera_stream, args=(right_camera_data, 'Normal Side View Mirror')) #for test
...
def update_side_mirror_angle():
    ...
    while True:
        ...
        if detect_signal and steering_angle >= 0.1:
